...
# SELECT
SELECT 문: 데이터베이스에서 데이터를 조회할 때 사용되는 문장입니다. 예를 들어, 특정 테이블에서 모든 열을 선택하고자 할 때 다음과 같이 사용합니다.
...
# UPDATE
  UPDATE 문: 테이블에서 이미 존재하는 레코드의 데이터를 수정할 때 사용됩니다. 예를 들어, 특정 조건을 만족하는 레코드의 데이터를 업데이트하고자 할 때 다음과 같이 사용합니다.
  ### UPDATE [테이블명]
  ### SET [기존컬럼명1] = [새로운 값], [기존컬럼명2] = [새로운값];

# DELETE
  DELETE 문: 테이블에서 특정 레코드를 삭제할 때 사용됩니다. 예를 들어, 특정 조건을 만족하는 레코드를 삭제하고자 할 때 다음과 같이 사용합니다.
  ### DELETE FROM [테이블명]
  
# JOIN
 JOIN 문: 둘 이상의 테이블을 연결하여 데이터를 가져올 때 사용됩니다. 예를 들어, 두 개의 테이블을 조인하여 관련된 데이터를 선택하고자 할 때 다음과 같이 사용합니다.
